...
export default class Interpreter {   
    ...
    constructor(filepath) {
        ...
        this.operations   = ["com", "prn", "brk", "con", "clr", 
                             ...
                             "time", "set", "join", "wait", "inp", "inpType"] 
 ...
 /**
   ...
   completeBrackets(line) {
    ...
    resolveLogic(comparison, input1, input2) {
        ...
        else if (this.constants.has(input1)) {input1 = this.constants.get(input1); isVariable[0] = true}
        if (this.variables.has(input2)) {input2 = this.variables.get(input2); isVariable[1] = true}
        else if (this.constants.has(input2)) {input2 = this.constants.get(input2); isVariable[1] = true}

        // arrays
        if (!isVariable[0] && input1.indexOf(":") !== -1) {
            let indexes1 = this.getArrayIndexes(input1)
            input1 = this.getArrayAtIndex(input1, indexes1)
        }
        if (!isVariable[1] && input2.indexOf(":") !== -1) {
    ...
    concatenate(args) {
        ...
        let firstArgument = args.slice(0, args.indexOf(" "))
        ...
        for (let i = 0; i < args.length; i++) {
            ...
            if (arg.startsWith(this.readStart)) {
                arg = arg.slice(1, arg.length-1)
            }

            if (arg.indexOf(":") !== -1) {
                let indexes = this.getArrayIndexes(arg)
                args[i] = this.getArrayAtIndex(arg, indexes)
                continue
            }

            if (this.variables.has(arg)) args[i] = this.variables.get(arg)
            else if (this.constants.has(arg)) args[i] = this.constants.get(arg)
            else if (this.arrays.has(arg)) args[i] = this.arrays.get(arg)
            else args[i] = arg
        }

        let reference = false
        if (!firstArgument.startsWith(this.readStart) && !firstArgument.endsWith(this.readEnd)) {
            reference = firstArgument
            firstArgument = this.variables.get(reference)
        }

        args = firstArgument + args.join('')
    ...
    onKeyStroke(args) {
        ...
        process.stdin.on('data', (key) => {
            if (this.inputLine === undefined) this.input.destroy()
    ...
    updateBenchmark(label) {
        ...
        if (benchmark !== undefined) {
            return (process.hrtime(benchmark)[1] / 1000000) 
        }
        // create benchmark
        else {
    ...
    insertVariablesInString(string) {
        ...
        while (string.indexOf(rs) !== -1 && string.indexOf(re) !== -1) {
            ...
            else if (this.constants.has(inBrackets)) value = this.constants.get(inBrackets)

            // arrays
            else if (this.arrays.has(inBrackets)) value = this.arrays.get(inBrackets).toString()
            else if (inBrackets.indexOf(":") !== -1 ) {
